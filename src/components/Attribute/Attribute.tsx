    ...
    <>
      ...
      <div className={variant === 'product' ? "d-flex" : "row"}>
        {attribute.items.map((item) => (
          <div
            key={item.id}
            data-testestid={variant === 'product' ? `product-attribute-${attribute.name.toLowerCase()}-${item.value.toLowerCase()}` : `product-attribute-${attribute.name.toLowerCase()}-${item.displayValue.toLowerCase()}`}
